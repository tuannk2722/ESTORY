...
- [ ] `/admin/**` không có `AppHeader` và luôn exact-admin. Các route workspace dùng `AdminShell`: desktop sidebar gập/mở; mobile/tablet navigation sheet; có Stories/Effects/Scene Library, lối về Reader, Admin identity và shared ThemeSwitcher. Admin chapter preview là route shell-neutral để Reader chiếm đúng viewport và scroll context.
...
- [ ] `genre` match exact public facet và kết hợp AND với `q`, không nhập vào search document. `listPublicGenreFacets(6)` lấy động từ Story published, đếm distinct Story, xếp số Story từ nhiều xuống ít với tie-break deterministic và UI giữ nguyên thứ tự; không hardcode taxonomy/derive từ page hiện tại. Genre chips wrap responsive, không clip/horizontal-scroll; `listPublicStories()` áp visibility public trong DB, cursor ổn định và chỉ trả card DTO + total/next cursor.
