...
- **Palette/Preset/Recipe mới cho Scene:** từ P3-14 không xây global ColorPalette catalog mới, ScenePreset importer/builder/catalog mới hoặc abstraction thay thế kiểu “Atmosphere Recipe”. Author không có accent picker, strength hay Advanced trong phiên bản đầu; Scene mới chỉ có Auto/Original theo `02` §2.9.
- **AI/server metadata cho Auto treatment:** P3-14 derive deterministic ở Author client và lưu resolved Scene snapshot. Không thêm AI call, Background color-metadata column/backfill hoặc server image decoder nếu client/CORS/performance chưa thất bại qua gate có bằng chứng và chưa có quyết định sản phẩm mới.
- **Bulk-migrate hoặc xóa compatibility sớm:** không rewrite Scene v1 sang v2, không drop `ColorPalette`/`ScenePreset`, provenance, parser hay sửa migration P3-04 trước reference audit P3-17 và forward migration được duyệt.
