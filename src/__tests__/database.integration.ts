...
async function run() {
  ...
  try {
    await assert.rejects(prisma.$transaction(async (tx) => {
      ...
      const story = await tx.story.create({ data: { slug: `foundation-${suffix}`, title: "Foundation", authorDisplayName: "Foundation author", searchTextNormalized: "foundation foundation author foundationfoundationauthor", description: "Transaction rollback test", genre: ["test"], authorId: user.id } });
