...
export default function AdminSceneLibraryError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div role="alert" className="rounded-2xl border border-[var(--color-destructive)]/45 bg-[var(--color-card)] p-6">
        <h1 className="font-display text-2xl font-bold">Không thể mở thư viện bối cảnh</h1>
        <p className="mt-2 text-sm text-[var(--color-muted-foreground)]">Dữ liệu quản trị chưa tải được. Tab và bộ lọc trên URL vẫn được giữ nguyên.</p>
