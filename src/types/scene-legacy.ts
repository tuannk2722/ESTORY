// JSON/editor compatibility types; target types are required snapshots in scene.ts.
// types/scene.ts
// Phase 2: Domain Model cho LegacyScene System (docs/02-data-schema.md mục 2.9 & docs/08-effects-and-scenes.md)
import { EffectConfig } from "./story";

// ─── 3 thư viện nguyên liệu — ADMIN quản lý (Phase 3) / seed JSON (Phase 1–2) ───

import type { BackgroundScope, BackgroundSource } from "./scene";
type BackgroundType = "image" | "video" | "gradient" | "particle_composition";

export interface LegacyBackgroundAsset {
  id: string;
  type: BackgroundType;
...
export interface LegacyScenePreset {
  id: string;
  label: string;                  // "Lễ rước dâu", "Trận thủy chiến"
  background_id: string;          // → LegacyBackgroundAsset.id
  palette_id: string;             // → LegacyColorPalette.id
  effects?: EffectConfig[];       // Danh sách hiệu ứng không gian êm dịu (nhạc nền loop, hạt mưa, đom đóm, tuyết rơi...)
  mood_tags: string[];
}

export interface LegacySceneLibraryData {
  backgrounds: LegacyBackgroundAsset[];
  palettes: LegacyColorPalette[];
  scenePresets: LegacyScenePreset[];
}

// ─── LegacyScene: AUTHOR tạo cho 1 truyện cụ thể, áp cho 1 dải block ───
